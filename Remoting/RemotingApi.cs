    ...
    class RemotingApi : MarshalByRefObject, IRemotingApi
    {
        ...
        public bool Init(int hbProcID)
        {
            ...
            if (profile != null)
            {
                ...
                profile.Log("Opened communication with HBRelogHelper  Id: {0}", hbProcID);
                return true;
            }
        ...
        public bool ReattachHB(int oldBbProcID, int newBbProcID)
        {
            Log.Write("Received request for reattach HB Id: {0}->{1}", oldBbProcID, newBbProcID);

            CharacterProfile profile = GetProfileByHbProcID(oldBbProcID);
            if (profile != null)
            {
                profile.TaskManager.HonorbuddyManager.ChangeBotProcess(newBbProcID);
                profile.Log("Changing communication with HBRelogHelper");
                return true;
            }
            Log.Write("Can't identify process Id: {0}", newBbProcID);
            return false;
        }
