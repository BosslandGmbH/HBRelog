...
namespace WinAuth
{
	/// <summary>Class that implements Battle.net Mobile Authenticator v1.1.0.</summary>
	public class BattleNetAuthenticator : Authenticator
	{
		/// <summary>Size of model string</summary>
		private const int MODEL_SIZE = 16;

		/// <summary>String of possible chars we use in our random model string</summary>
		private const string MODEL_CHARS = " ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz01234567890";

		/// <summary>Buffer size used on Http responses</summary>
		private const int RESPONSE_BUFFER_SIZE = 64;

		/// <summary>Expect size of return data from enroll</summary>
		private const int ENROLL_RESPONSE_SIZE = 45;

		/// <summary>Expected size of return data from time sync</summary>
		private const int SYNC_RESPONSE_SIZE = 8;

		/// <summary>Buffer size used on Restore call</summary>
		private const int RESTOREINIT_BUFFER_SIZE = 32;

		/// <summary>Buffer size used on Restore Validation call</summary>
		private const int RESTOREVALIDATE_BUFFER_SIZE = 20;

		/// <summary>Number of digits in code</summary>
		private const int CODE_DIGITS = 8;

		/// <summary>The public key modulus used to encrypt our data</summary>
		...
		/// <summary>Public key exponent used to encrypt our data</summary>
		private const string ENROLL_EXPONENT =
			"0101";

		/// <summary>Number of minutes to ignore syncing if network error</summary>
		private const int SYNC_ERROR_MINUTES = 5;

		/// <summary>URLs for all mobile services</summary>
		...
		/// <summary>Set of ISO3166 EU countries</summary>
		...
		/// <summary>Set of ISO3166 KR countries</summary>
		...
		/// <summary>URL for GEO IP lookup to determine region</summary>
		private static string GEOIPURL = "http://geoiplookup.wikimedia.org";

		/// <summary>Time of last Sync error</summary>
		private static DateTime _lastSyncError = DateTime.MinValue;

		#region Authenticator data

		/// <summary>Region for authenticator taken from first 2 chars of serial</summary>
		public string Region { get { return (string.IsNullOrEmpty(Serial) == false ? Serial.Substring(0, 2) : string.Empty); } }
		...
		/// <summary>We can check if the restore code is valid and rememeber so don't have to do it again</summary>
		public bool RestoreCodeVerified { get; set; }

		/// <summary>Get the restore code for an authenticator used to recover a lost authenticator along with the serial number.</summary>
		/// <returns>restore code (10 chars)</returns>
		public string RestoreCode { get { return BuildRestoreCode(); } }
		...
		/// <summary>Enroll the authenticator with the server.</summary>
		public void Enroll()
		{
			// default to US
...
#if DEBUG
		/// <summary>Debug version of enroll that just returns a known test authenticator</summary>
		...
		public void Enroll(bool testmode)
		{
			...
			else
			{
				...
				SecretKey = new byte[20]
							{
								0x7B, 0x0B, 0xFA, 0x82, 0x30, 0xE5, 0x44, 0x24, 0xAB, 0x51, 0x77, 0x7D, 0xAD, 0xBF, 0xD5, 0x37, 0x41, 0x43, 0xE3,
								0xB0
							};
				...
				using (HttpWebResponse response = (HttpWebResponse) request.GetResponse())
				{
					...
					using (MemoryStream ms = new MemoryStream())
					{
						...
						using (Stream bs = response.GetResponseStream())
						{
							...
							{
								throw new InvalidSyncResponseException(
									string.Format("Invalid response data size (expected " + SYNC_RESPONSE_SIZE + " got {0}", responseData.Length));
							}
						}
					}
				}
		...
		/// <summary>Restore an authenticator from the serial number and restore code.</summary>
		...
		public void Restore(string serial, string restoreCode)
		{
				...
				using (HttpWebResponse response = (HttpWebResponse) request.GetResponse())
				{
					...
					using (MemoryStream ms = new MemoryStream())
					{
						using (Stream bs = response.GetResponseStream())
						{
							...
							{
								throw new InvalidRestoreResponseException(
									string.Format("Invalid response data size (expected 32 got {0})", challenge.Length));
							}
						}
					}
				}
			...
			catch (WebException we)
			{
				...
				{
					throw new InvalidRestoreResponseException(
						string.Format("Error communicating with server: {0} - {1}", code, ((HttpWebResponse) we.Response).StatusDescription));
				}
			}
				...
				using (HttpWebResponse response = (HttpWebResponse) request.GetResponse())
				{
					...
					using (MemoryStream ms = new MemoryStream())
					{
						using (Stream bs = response.GetResponseStream())
						{
							...
							{
								throw new InvalidRestoreResponseException(
									string.Format("Invalid response data size (expected " + RESTOREVALIDATE_BUFFER_SIZE + " got {0})", secretKey.Length));
							}
						}
					}
				}
			...
			if (serial.Length == 14)
			{
				Serial = serial.Substring(0, 2).ToUpper() + "-" + serial.Substring(2, 4) + "-" + serial.Substring(6, 4) + "-" +
						serial.Substring(10, 4);
			}
			else
			{
		...
		/// <summary>Read any extra tags from the Xml</summary>
		...
		/// <summary>Add extra tags into the XmlWriter</summary>
		...
		/// <summary>Get the base mobil url based on the region</summary>
		...
		///     secret key, which is then specially encoded to alphanumerics.
		/// </summary>
		...
		/// <summary>Create a random Model string for initialization to armor the init string sent over the wire</summary>
		...
		/// <summary>Convert a byte to a char but with appropriate mapping to exclude I,L,O and S.</summary>
